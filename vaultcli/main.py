...
def update():
    ...
    url = "https://api.github.com/repos/YOUR_USERNAME/vaultcli/releases/latest"
    try:
    ...
    if sys.platform.startswith("linux"):
        ...
        asset_name = "vault-macos"
    elif sys.platform == "win32":
        asset_name = "vault-windows.exe"
    else:
    ...
    try:
        binary = requests.get(download_url, timeout=60).content
    except requests.RequestException as exc:
        print(f"❌ Could not download update: {exc}")
        return

    # overwrite current executable
...
def main(argv=None):
    ...
    known_commands = {"init", "login", "signup", "logout", "get", "put"}
